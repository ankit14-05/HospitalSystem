import React from 'react';
import { FileText, Briefcase, FileSignature } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export default function AdminStaffDashboard() {
  const { user } = useAuth();
  
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl p-6 text-white shadow-lg">
          <h1 className="text-2xl font-bold mb-2">Welcome, {user?.username || 'Admin Staff'}</h1>
          <p className="text-amber-50 opacity-90">Administrative & Clerical Ops. Manage hospital records, billing verifications, and general paperwork.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-amber-50 flex items-center justify-center text-amber-500">
              <FileText size={24} />
            </div>
            <div>
              <p className="text-slate-500 text-sm font-medium">Pending Documents</p>
              <h3 className="text-2xl font-bold text-slate-800">18</h3>
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500">
              <Briefcase size={24} />
            </div>
            <div>
              <p className="text-slate-500 text-sm font-medium">Active Requests</p>
              <h3 className="text-2xl font-bold text-slate-800">5</h3>
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-500">
              <FileSignature size={24} />
            </div>
            <div>
              <p className="text-slate-500 text-sm font-medium">Files Processed</p>
              <h3 className="text-2xl font-bold text-slate-800">42</h3>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-96 flex flex-col items-center justify-center text-slate-500">
          <FileText size={48} className="text-amber-200 mb-4" />
          <h2 className="text-xl font-bold text-slate-700">Digital Filing Cabinet</h2>
          <p className="max-w-md text-center text-sm mt-2">
            Secure electronic medical records (EMR) document handling is coming soon.
          </p>
        </div>

      </div>
    </div>
  );
}
