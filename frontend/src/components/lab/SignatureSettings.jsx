...
import { 
  Upload, Save, Image as ImageIcon, Check, 
  Layout, MousePointer2, Settings2, Info 
} from "lucide-react";
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {[
                    ...
                    <button
                      key={opt.id}
                      onClick={() => setSettings({ ...settings, SignaturePreference: opt.id })}
                      className={`
                        w-full p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-4
                        ${settings.SignaturePreference === opt.id 
                          ? 'border-teal-500 bg-teal-50/50 ring-4 ring-teal-500/10' 
                          : 'border-slate-100 hover:border-slate-200 hover:bg-slate-50'}
                      `}
                    >
                      <div className={`p-3 rounded-xl ${settings.SignaturePreference === opt.id ? 'bg-teal-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                        <opt.icon size={20} />
                      </div>
                      <div className="flex-1">
                        <p className={`font-bold text-sm ${settings.SignaturePreference === opt.id ? 'text-teal-900' : 'text-slate-700'}`}>{opt.label}</p>
                        <p className="text-[11px] text-slate-400 font-medium mt-0.5">{opt.desc}</p>
                      </div>
                      {settings.SignaturePreference === opt.id && <Check className="text-teal-500" size={18} />}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col">
              <div 
                className={`
                  ...
                  ${previewUrl || settings.SignatureImagePath ? 'border-teal-200 bg-teal-50/20' : 'border-slate-200 hover:border-teal-300 hover:bg-slate-50'}
                `}
              >
                {(previewUrl || settings.SignatureImagePath) ? (
                  <div className="w-full h-full flex flex-col items-center justify-center">
                    <img 
                      src={previewUrl || (import.meta.env.VITE_API_URL || '') + settings.SignatureImagePath} 
