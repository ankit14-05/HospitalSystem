...
const rejectLegacyDoctorScheduleEdit = (_req, _res, next) =>
  next(
    new AppError(
      'Doctor self-schedule editing has been disabled. Schedule assignments are managed by admin, and OPD slots are published by the OPD manager.',
      403
    )
  );
...
router.post('/my-schedule', authorize('doctor','admin','superadmin'), rejectLegacyDoctorScheduleEdit);

// PUT /doctors/my-schedule/:id
router.put('/my-schedule/:id', authorize('doctor','admin','superadmin'), rejectLegacyDoctorScheduleEdit);

// DELETE /doctors/my-schedule/:id  (soft delete)
router.delete('/my-schedule/:id', authorize('doctor','admin','superadmin'), rejectLegacyDoctorScheduleEdit);
...
router.get('/activity-types',          authenticate, authorize('admin','superadmin','auditor','doctor','opdmanager','opd_manager'), dsCtrl.getActivityTypes);
...
router.get('/day-overview',            authenticate, authorize('admin','superadmin','auditor','opdmanager','opd_manager'),          dsCtrl.getDayOverview);
router.post('/opd-slot-sessions',      authenticate, authorize('opdmanager','opd_manager'),                                         dsCtrl.createOpdSlotSession);
router.get('/rooms',                   authenticate, authorize('admin','superadmin','auditor','opdmanager','opd_manager'),          dsCtrl.getRooms);
router.get('/locations',               authenticate, authorize('admin','superadmin','auditor','opdmanager','opd_manager'),          dsCtrl.getLocations);
router.get('/shift-templates',         authenticate, authorize('admin','superadmin','auditor','opdmanager','opd_manager'),          dsCtrl.getShiftTemplates);
router.get('/weekly-templates',        authenticate, authorize('admin','superadmin','auditor','opdmanager','opd_manager'),          dsCtrl.getWeeklyTemplates);
router.post('/weekly-templates',       authenticate, authorize('admin','superadmin'),                                               dsCtrl.saveWeeklyTemplate);
router.delete('/weekly-templates',     authenticate, authorize('admin','superadmin'),                                               dsCtrl.deleteWeeklyTemplate);
